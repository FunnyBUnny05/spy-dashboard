...
export function VolRegimePanel({ timeseries }: Props) {
  ...
  const isAntiPredictive = stats.confidence === 'wrong-signed';

  const RHO_FROM_STATS: Record<string, number> = {
    low:      stats.rhoLow,
    mid_low:  stats.rhoMidLow,
    mid_high: stats.rhoMidHigh,
    high:     stats.rhoHigh,
  };
  ...
  return (
    <div className="chart-box" style={{ marginTop: 16 }}>
      ...
      <div className={`callout ${isAntiPredictive ? 'callout-warn' : 'callout-info'}`}
           style={{ marginBottom: 12 }}
           role={isAntiPredictive ? 'alert' : 'status'}>
        ...
        OOS ρ in this regime: <span style={{ color: currentRho.color, fontWeight: 700 }}>
          {RHO_FROM_STATS[stats.current] >= 0 ? '+' : ''}{RHO_FROM_STATS[stats.current].toFixed(3)}
        </span>
      ...
      <table>
        ...
        <tbody>
          {[
            { key: 'low',      range: '<10%',       meanVol: '8.2%',  n: 34 },
            { key: 'mid_low',  range: '10–13.5%',   meanVol: '11.6%', n: 34 },
            { key: 'mid_high', range: '13.5–18.5%', meanVol: '15.2%', n: 33 },
            { key: 'high',     range: '≥18.5%',     meanVol: '22.5%', n: 34 },
          ].map(row => {
            const isCurrent = row.key === stats.current;
            const rho = RHO_FROM_STATS[row.key];
            return (
